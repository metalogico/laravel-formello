<div class="mb-4">
    ...
    @if (isset($config['icon']) || isset($config['prefix']) || isset($config['suffix']))
        <div class="mt-1 flex rounded-md">
            ...
            <input name="{{ $name }}" value="{{ $value }}"
                class="block w-full min-w-0 flex-1 rounded-none {{ (isset($config['prefix']) || isset($config['icon'])) ? 'rounded-r-md' : 'rounded-md' }} border-gray-300 focus:border-blue-300 focus:ring-blue-300 {{ $errors ? 'border-red-500 focus:border-red-500 focus:ring-red-300' : '' }} {{ $config['attributes']['class'] ?? '' }}"
    ...
    @else
        <input name="{{ $name }}" value="{{ $value }}"
            class="mt-1 block w-full rounded-md border-gray-300 focus:border-blue-300 focus:ring-blue-300 {{ $errors ? 'border-red-500 focus:border-red-500 focus:ring-red-300' : '' }} {{ $config['attributes']['class'] ?? '' }}"
            @foreach ($config['attributes'] as $attr => $attrValue)
                @if ($attr !== 'class') {{ $attr }}="{{ $attrValue }}" @endif
            @endforeach>
    @endif
