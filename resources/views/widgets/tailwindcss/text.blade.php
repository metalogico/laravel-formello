<div class="mb-4">
    ...
    @elseif ($hasOverlayIcon)
        <div class="relative mt-1">
            <input name="{{ $name }}" value="{{ old($name, $value) }}"
                class="w-full pl-10 pr-3 py-2 border border-gray-300 rounded-md bg-white text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-1 focus:ring-blue-300 focus:border-blue-500 transition {{ $errorState }} {{ $config['attributes']['class'] ?? '' }}"
                @foreach ($config['attributes'] as $attr => $attrValue)
                    @if ($attr !== 'class') {{ $attr }}="{{ $attrValue }}" @endif
                @endforeach>
    ...
    @else
        <input name="{{ $name }}" value="{{ old($name, $value) }}"
            class="mt-1 block w-full rounded-md bg-white text-gray-900 placeholder:text-gray-400 border border-gray-300 px-3 py-2 focus:outline-none focus:ring-1 focus:ring-blue-300 focus:border-blue-500 transition {{ $errorState }} {{ $config['attributes']['class'] ?? '' }}"
            @foreach ($config['attributes'] as $attr => $attrValue)
                @if ($attr !== 'class') {{ $attr }}="{{ $attrValue }}" @endif
            @endforeach>
    @endif
