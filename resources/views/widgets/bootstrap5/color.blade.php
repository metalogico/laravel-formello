<div class="form-group mb-3">
    ...
    @if (isset($config['icon']) || isset($config['prefix']) || isset($config['suffix']))
        <div class="input-group">
            ...
            @endif
            <input name="{{ $name }}" value="{{ $value }}" class="form-control {{ $config['attributes']['class'] ?? '' }} @if ($errors) is-invalid @endif" @foreach ($config['attributes'] as $attr => $attrValue) @if($attr !== 'class') {{ $attr }}="{{ $attrValue }}" @endif @endforeach>
